...
class SearchConfig(BaseModel):
    ...
    candidate_pool_top_k: int = Field(
        default=50,
        description="普通查询内部粗召/RRF候选池大小；最终返回数量仍由 default_top_k 或 search(top_k=...) 控制",
    )
    context_candidate_pool_top_k: int = Field(
        default=150,
        description="函数记忆/实现查找且带具体上下文时的内部候选池大小",
    )
    issue_candidate_pool_top_k: int = Field(
        default=200,
        description="Issue/SWE 模式的候选池大小，需更宽以容纳实现/配置/文档跨族 gold 文件",
    )
    issue_lane_top_k: int = Field(
        default=20,
        description="Issue ?? BM25/??/???? RRF ????? top_k?v11 ????? candidate_pool_top_k*3=600 ??????",
    )
    issue_aux_pool_top_k: int = Field(
        default=20,
        description="Issue ?? title/hint ?? top_k ???v11 ?????? title ???????",
    )
    ...
    issue_rerank_top_k: int = Field(default=10, description="Issue/SWE ????????????20???10")
